 ...
 */
void* tree_sitter_sdevice_external_scanner_create() {
    return NULL;
}
 ...
 * 我们的扫描器是无状态的，所以返回 0。
 */
unsigned tree_sitter_sdevice_external_scanner_serialize(void* payload, char* buffer) {
    return 0;
}
 ...
 * 我们的扫描器是无状态的，所以为空实现。
 */
void tree_sitter_sdevice_external_scanner_deserialize(void* payload, const char* buffer, unsigned length) {
    // 无需恢复状态
}
 ...
 */
bool tree_sitter_sdevice_external_scanner_scan(void* payload, TSLexer* lexer, const bool* valid_symbols) {
